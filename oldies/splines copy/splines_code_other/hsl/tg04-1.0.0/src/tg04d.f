...
C
C
      SUBROUTINE TG04AD(N,X,K,NORM,XVALUE,JINT,V,VINT)
C      ********************************************************
C                            PURPOSE
C      ********************************************************
C
...
C     ..
C     .. Array Arguments ..
      DOUBLE PRECISION V(*),VINT(*),X(*)
C     ..
C     .. Local Scalars ..
      DOUBLE PRECISION DK,E1,E2,E3,XLEFT,XRITE
...
C     .. Intrinsic Functions ..
      INTRINSIC DBLE,FLOAT,MAX0,MIN0
C     ..
C     .. Common blocks ..
      COMMON /TG04BD/LP,IFAIL
      INTEGER IFAIL,LP
C     ..
C     .. Save statement ..
      SAVE /TG04BD/
...
C      ********************************************************
  190 JJ1 = N - JINT
      ...
      IF (NORM.EQ.1) VINT(1) = ONE/DK
      IF (NORM.EQ.2) VINT(1) = (X(JINT)-X(JINTMK))/DK
      RETURN
C
C      ********************************************************
...
C      ********************************************************
  300 DO 320 J = 1,K
        ...
        IF (NORM.EQ.1) VINT(J) = VINT(J)/DK
        IF (NORM.EQ.2) VINT(J) = (X(JINTPJ)-X(JTPJMK))*VINT(J)/DK
        GO TO 320
...
C      DIAGNOSTIC PRINTING
  ...
  380 FORMAT (1X,19HMESSAGE FROM TG04AD,3X,
     +       41HNORM WAS .NE.1 OR 2 AND HAS BEEN SET TO 1)
  390 FORMAT (1X,19HMESSAGE FROM TG04AD,3X,2HK=,I4,
     +       18H AND IS NOT .GT. 1)
  400 FORMAT (1X,19HMESSAGE FROM TG04AD,3X,2HN=,I4,
     +       20H AND IS NOT .GE.K+1=,I4)
  410 FORMAT (1X,19HMESSAGE FROM TG04AD,3X,
     +       43HTHE DATA POINTS ARE NOT IN ASCENDING ORDER.)
  420 FORMAT (1X,19HMESSAGE FROM TG04AD,3X,10HMORE THAN ,I4,
