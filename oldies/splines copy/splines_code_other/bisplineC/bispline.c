/*
 * This is a translation of TG04AD from the HSL library, written in Fortran by
 * P.W. Gaffney in 1976. I removed the goto's and the diagnostic output, but the
 * aroma of spaghetti is still overwhelming.
 *
...
void bisplineBasisC(const int *, const double *, const int *, const int *,
...
void bisplineBasisC(const int *nknots, const double *knots, const int *order,
    ...
    for (int i = 1; i <= m; i++) {
        ...
        for (int j = 1; j <= k; j++) {
            ...
            ibasis[MINDEX(i, j1, m)] = vint[INDEX(j)];
        }
        free(v);
        free(vint);
    }
