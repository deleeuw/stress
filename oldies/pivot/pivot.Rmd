...
```{r pivorder, echo = FALSE}
a[1,1] <- 0
mprint(a)
```
If we pivot on the first four elements we get
```{r pivzero, echo = FALSE}
h<-pivotRC(a,1:4)
mprint(h$a)
```
and pivots are done in the order `r h$pivot`. 

Another example is the rank 2 matrix
```{r pivsing}
mprint(a<-tcrossprod(matrix(c(1,1,1,1,1,-1,-1,1),4,2))/2)
```
```{r pivsing_do, echo = FALSE}
h<-pivotRC(a,1:4)
```
A complete pivot uses the order `r h$pivot` and finds 
```{r pivot_a}
mprint(h$a)
```
Note that if the function `pivotOneRC()` is asked to pivot on a zero diagonal element, it refuses and just returns its
argument. Thus if zero diagonal elements are encountered, it is just as if the corresponding indices are not in the sequence, and we actually pivot on a shorter sequence. `pivotRC()` returns an indicator of the pivots that are skipped, which for our last example is `r h$skip`.

Also note that order-invariance for `pivotRC()` is no longer true if pivots are skipped. Consider

```{r skipper, echo = FALSE}
a<-matrix (0, 4, 4)
a[,4]<-a[4,]<-1
mprint(a)
```
...
```{r degen, echo = FALSE}
mprint (a <- rbind(cbind(matrix (c(0,1,1,0), 2, 2),diag(2)),cbind(diag(2),diag(2))))
```
...
```{r display_code_c, eval = FALSE, engine = "Rcpp"}
