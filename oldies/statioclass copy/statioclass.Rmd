...
$$ **`r result_nums("differentiable", display = "full")`** $\sigma(\bullet)$ is differentiable at at $X$ if and only if $d_{ij}(X)>0$ for all $i<j$ with $w_{ij}\delta_{ij}>0$. In that case $$
\mathcal{D}\sigma(X)=(V-B(X))X.
$$ **`r result_nums("diffstat", display = "full")`** If $\sigma(\bullet)$ is differentiable at the stationary point $X$ then $(V-B(X))X=0$, or, in fixed point form, $X=V^+B(X)X$.
...
**`r result_nums("bounded", display = "full")`** At a local minimum point $X$ of $\sigma(\bullet)$ we have $\eta(X)\leq 1$.
...
**`r result_nums("maxima", display = "full")`** $X=0$ is the unique local maximum point of $\sigma(\bullet)$ and $\sigma(0)=1$ is the unique local maximum.
...
**`r result_nums("unbounded", display = "full")`** $\sigma(\bullet)$ is unbounded and consequently has no global maximum.
...
```{r first_array, echo = FALSE, cache = FALSE}
aa <- bb <- seq (-.60, .60, length = 100)
z <- matrix (0, 100, 100)
for (i in 1:100) for (j in 1:100)
  z[i, j]<- stress ((aa[i] * x) + (bb[j] * y), delta)
```
...
s <- sum(w * delta ^ 2)
delta <- delta / sqrt (s)
x <- matrix (c(0,1,-1,0), 4, 1, byrow = TRUE)
y <- matrix (c(1,0,0,-1), 4,1, byrow = TRUE)
x <- apply (x, 2, function (z) z - mean (z))
y <- apply (y, 2, function (z) z - mean (z))
```
...
```{r first_array3, echo = FALSE, cache = FALSE}
aa <- bb <- seq (-.60, .60, length = 100)
z <- matrix (0, 100, 100)
for (i in 1:100) for (j in 1:100)
  z[i, j]<- stress ((aa[i] * x) + (bb[j] * y), delta)
...
```{r global_perspective3, fig.align= "center", fig.width = 10, fig.height = 10, echo = FALSE, cache = FALSE}
par(pty="s")
persp(aa, bb, z, col = "RED", xlab = "theta_1", ylab = "theta_2", zlab = "stress")
```
...
**`r result_nums("expansion", display = "full")`** \begin{multline}
...
**`r result_nums("anti", display = "full")`** Suppose $Y=XT$, with $T$ antisymmetric, so that $X+\epsilon Y=X(I+\epsilon T)$. Then \begin{equation}
\sigma(X+\epsilon Y)=\sigma(X)-\epsilon\sum_{d_{ij}(X)=0}w_{ij}\delta_{ij}d_{ij}(Y)
+\frac12\epsilon^2\text{tr}\ Y'(V-B(X))Y+o(\epsilon^2)
\end{equation}

**`r result_nums("singular", display = "full")`** Suppose $\underline{X}=[X\mid 0]$ and $\underline{X}=[0\mid Y]$ so that $\underline{X}+\epsilon\underline{Y}=[X\mid\epsilon Y]$. Here $\underline{X}$ and $\underline{Y}$ are $n\times p$, $X$ is $n\times r$, with $r<p$, and $Y$ is $n\times(p-r)$. Then \begin{equation}
\sigma(\underline{X}+\epsilon \underline{Y})=\sigma(X)-\epsilon\sum_{d_{ij}(X)=0}w_{ij}\delta_{ij}d_{ij}(Y)
+\frac12\epsilon^2\text{tr}\ Y'(V-B(X))Y+o(\epsilon^2)
\end{equation}

**`r result_nums("singular", display = "full")`** Suppose $\underline{X}=[X\mid 0]$ and $\underline{X}=[Z\mid Y]$ so that $\underline{X}+\epsilon\underline{Y}=[X+\epsilon Z\mid\epsilon Y]$. Here $\underline{X}$ and $\underline{Y}$ are $n\times p$, $X$ is $n\times r$, with $r<p$, and $Y$ is $n\times(p-r)$. Then \begin{equation}
\sigma(\underline{X}+\epsilon \underline{Y})=\sigma(X)-\epsilon\sum_{d_{ij}(X)=0}w_{ij}\delta_{ij}d_{ij}(Y)
+\frac12\epsilon^2\text{tr}\ Y'(V-B(X))Y+o(\epsilon^2)
\end{equation}
...
**`r result_nums("first", display = "full")`** \begin{equation}
d\sigma(X;Y)=
\text{tr}\ X'(V-B(X))Y-\sum_{d_{ij}(X)=0}w_{ij}\delta_{ij}d_{ij}(Y).
\end{equation}

**`r result_nums("second", display = "full")`** \begin{equation}
d^2\sigma(X;Y)=
...
**`r result_nums("locmin", display = "full")`** If $X$ is a local minimum point of $\sigma(\bullet)$ then
...
**`r result_nums("hesmin", display = "full")`** If $X$ is a local minimum point of $\sigma(\bullet)$ then $\overrightarrow{V}-H(X)$ is positive semi-definite.

**`r result_nums("hessin", display = "full")`** If $[X\mid 0]$ is a local minimum point of $\sigma(\bullet)$ then

1.  $d_{ij}(X)>0$ for all $i<j$ with $w_{ij}\delta_{ij}>0$.
2.  $(V-B(X))X=0$.
3.  $V-B(X)$ is positive semi-definite.

**`r result_nums("homogen", display = "full")`** If $X$ is a stationary point of $\sigma(\bullet)$ then $\eta(X)\leq 1$.
