---
title: "Stationary Points in Multidimensional Scaling"
author: "Jan de Leeuw"
date: '`r paste("First created on May 21, 2019. Last update on", format(Sys.Date(),"%B %d, %Y"))`'
output:
  html_document:
    keep_md: yes
    number_sections: yes
...
result_nums <- captioner (prefix = "Result")
result_nums(name = "dc", caption = "[DC]", display = FALSE)
result_nums (name = "bounded", caption = "[Bounded]", display = FALSE)
result_nums (name = "maxima", caption = "[Local Maxima]", display = FALSE)
result_nums (name = "unbounded", caption = "[Unbounded]", display = FALSE)
result_nums (name = "expansion", caption = "[Quadratic]", display = FALSE)
result_nums (name = "anti", caption = "[Antisymmetric]", display = FALSE)
result_nums (name = "singular", caption = "[Singular]", display = FALSE)
result_nums (name = "first", caption = "[First Directional Derivatives]", display = FALSE)
result_nums (name = "second", caption = "[Second Directional Derivatives]", display = FALSE)
result_nums (name = "locmin", caption = "[Local Minimum 1]", display = FALSE)
result_nums (name = "hesmin", caption = "[Local Minimum 2]", display = FALSE)
result_nums (name = "hessin", caption = "[Local Minimum 3]", display = FALSE)
figure_nums <- captioner (prefix ="Figure")
figure_nums (name = "stress", caption = "Picture of Stress", display = FALSE)
```
...
The problem we consider in this paper is minimization of the least squares loss function
\begin{equation}\label{E:stress}\sigma(X)=\frac12\mathop{\sum}_{1\leq i<j\leq n} w_{ij}(\delta_{ij}-d_{ij}(X))^2
\end{equation}
over all $n\times p$ matrices $X$.  We follow @kruskal_64a and call $\sigma(X)$ the *stress* of *configuration* $X$. Minimizing stress over p-dimensional configurations is the *pMDS problem*.

Two important special cases of pMDS are Unidimensional Scaling, which is 1MDS, and Full-dimensional Scaling, which is (n-1)MDS. Because of their importance, they also have their very own acronyms UDS and FDS.

In $\eqref{E:stress}$ the matrices of *weights* $W=\{w_{ij}\}$ and *dissimilarities* $\Delta=\{\delta_{ij}\}$ are symmetric, non-negative, and hollow (zero diagonal). The matrix-valued function $D(X)=\{d_{ij}(X)\}$ contains Euclidean distances between the rows of the configuration $X$, which are the coordinates of $n$ *points* in $\mathbb{R}^p$. Thus  mit is also symmetric, non-negative, and hollow. 

In pMDS we minimize stress. This means that we are trying to find the global minimum over p-dimensional configurations. In practice, however, our algorithms find local minima, which may or may not be global. In this paper we will summarize what we know about global and local minima, as well as about local maxima and saddle points. 
...
$$
Additionally $\rho(\bullet)$ is differentiable at $X$ if and only if $d_{ij}(X)>0$ for all
$i<j$ with $w_{ij}\delta_{ij}>0$. In that case
$$
\mathcal{D}\rho(X)=B(X)X.
$$
**`r result_nums("differentiable", display = "full")`**
$\sigma(\bullet)$ is differentiable at $X$ if and only if $d_{ij}(X)>0$ for all
$i<j$ with $w_{ij}\delta_{ij}>0$. In that case
$$
\mathcal{D}\sigma(X)=(V-B(X))X.
$$
**`r result_nums("diffstat", display = "full")`**
If $\sigma(\bullet)$ is differentiable at a stationary point $X$ then $(V-B(X))X=0$, or, in fixed point form, $X=V^+B(X)X$.

Diferentiability of $\sigma(\bullet)$ at $X$ only hinges on whether the $d_{ij}(X)$ are positive or zero. Thus if $\sigma(\bullet)$ is differentiable, then it 
is  twice differentiable as well (and three times differentiable, and so on).

$$
\sum_{1\leq i<j\leq n}w_{ij}(\delta_{ij}-d_{ij}(X))\frac{1}{d_{ij}(X)}A_{ij}X=\\
\sum_{1\leq i<j\leq n}w_{ij}(\delta_{ij}-d_{ij}(X))(e_i-e_j)\frac{(x_i-x_j)'}{d_{ij}(X)}
$$
$$
\|\mathcal{D}\sigma(X)\|\leq\sum_{1\leq i<j\leq n}w_{ij}|\delta_{ij}-d_{ij}(X)|\|(e_i-e_j)\frac{(x_i-x_j)'}{d_{ij}(X)}\|=\sqrt{2}\sum_{1\leq i<j\leq n}w_{ij}|\delta_{ij}-d_{ij}(X)|
$$
# DC Functions
...
**`r result_nums("bounded", display = "full")`**
At a local minimum point $X$ of $\sigma(\bullet)$ we have $\eta(X)\leq 1$.

**Proof:** 
Homogeneity gives $\sigma(\lambda X)=1-\lambda\rho(X)+\frac12\lambda^2\eta^2(X)$.
If $X$ is a local minimum then the minimum over $\lambda$ is attained for $\lambda=1$, i.e.
we must have $\rho(X)=\eta^2(X)$. By Cauchy-Schwartz $\rho(X)\leq\eta(X)$ for all $X$, and thus at a local minimum $\eta^2(X)\leq\eta(X)$, i.e. $\eta(X)\leq 1$. $\blacksquare$

**`r result_nums("maxima", display = "full")`**
$X=0$ is the unique local maximum point of $\sigma(\bullet)$ and $\sigma(0)=1$ is the unique local maximum.

**Proof:** This is because
$\sigma(\lambda X)=1-\lambda\rho(X)+\frac12\lambda^2\eta^2(X)$ 
is a convex quadratic in $\lambda$. The only maximum on the ray through $X$ occurs at the boundary $\lambda=0$. $\blacksquare$


**`r result_nums("unbounded", display = "full")`**
$\sigma(\bullet)$ is unbounded and consequently has no global maximum.

**Proof:**  $\blacksquare$

Note that the unboundedness of stress also follows from the proof of result `r result_nums("maxima", display = "n")`.



# Picture

Here is a picture of stress on a two-dimensional subspace which illustrates both result `r result_nums("maxima", display = "n")` and result `r result_nums("coercive", display = "n")`.

```{r stress, fig.align = "center",, out.width = "75%", echo = FALSE}
knitr::include_graphics("stress.png")
```
<hr>
<center>
`r figure_nums ("stress", display = "full")`
</center>
<hr>
...
\end{equation}
Equations $\eqref{E:dd}$, $\eqref{E:dg}$, and $\eqref{E:d0}$ combine in a straightforward way to an expansion of $\sigma(\bullet)$ at $X$ in direction $Y$.

**`r result_nums("expansion", display = "full")`**
\begin{multline}
...
**`r result_nums("anti", display = "full")`**
...
**`r result_nums("singular", display = "full")`**
Suppose $\underline{X}=[X\mid 0]$ and $\underline{X}=[0\mid Y]$ so that $\underline{X}+\epsilon\underline{Y}=[X\mid\epsilon Y]$. Here $\underline{X}$ and $\underline{Y}$ are $n\times p$, $X$ is $n\times r$, with $r<p$, and $Y$ is $n\times(p-r)$. Then
\begin{equation}
...
**`r result_nums("singular", display = "full")`**
Suppose $\underline{X}=[X\mid 0]$ and $\underline{X}=[Z\mid Y]$ so that $\underline{X}+\epsilon\underline{Y}=[X+\epsilon Z\mid\epsilon Y]$. Here $\underline{X}$ and $\underline{Y}$ are $n\times p$, $X$ is $n\times r$, with $r<p$, and $Y$ is $n\times(p-r)$. Then
\begin{equation}
\sigma(\underline{X}+\epsilon \underline{Y})=\sigma(X)-\epsilon\sum_{d_{ij}(X)=0}w_{ij}\delta_{ij}d_{ij}(Y)
+\frac12\epsilon^2\text{tr}\ Y'(V-B(X))Y+o(\epsilon^2)
\end{equation}
...
The expansion in result `r result_nums("expansion", display = "n")` immediately gives the first-order and second-order directional derivatives.

**`r result_nums("first", display = "full")`**
\begin{equation}
d\sigma(X;Y)=
\text{tr}\ X'(V-B(X))Y-\sum_{d_{ij}(X)=0}w_{ij}\delta_{ij}d_{ij}(Y).
\end{equation}

**`r result_nums("second", display = "full")`**
\begin{equation}
d^2\sigma(X;Y)=
...
Note that result `r result_nums("anti", display = "n")` imply that
...
**`r result_nums("hesmin", display = "full")`**
If $X$ is a local minimum point of $\sigma(\bullet)$ then $\overrightarrow{V}-H(X)$ is positive semi-definite.

**`r result_nums("hessin", display = "full")`**
If $[X\mid 0]$ is a local minimum point of $\sigma(\bullet)$ then 

1. $d_{ij}(X)>0$ for all $i<j$ with $w_{ij}\delta_{ij}>0$.
2. $(V-B(X))X=0$.
3. $V-B(X)$ is positive semi-definite.

**`r result_nums("homogen", display = "full")`**
If $X$ is a stationary point of $\sigma(\bullet)$ then $\eta(X)\leq 1$.
...
while for $d_{ij}(X)=0$
$$
\partial d_{ij}(X)=\text{co}\{(e_i-e_j)u'\mid u'u=1\},
$$
