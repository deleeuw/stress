...
We start with ordinary (total) differentiation of stress and then discuss, in considerable detail, the case of zero distances. A quick reminder of terminology and notation. The*derivative* of stress at configuration $X$ is $\mathcal{D}\sigma(X)$. It is a linear transformation of configuration space $\mathbb{R}^{n\times p}$ into
$\mathbb{R}$. The derivative at $X$ evaluated at configuration $Y$ is
$\mathcal{D}\sigma(X)(Y)$, which also defines the *differential* $\delta(X;Y)$.
The *partial derivatives* at $X$ are $\mathcal{D}\sigma(X)(E_s)=\delta(X,E_s)$, where the  $E_s$ are a basis for configuration space. The matrix of partial derivatives is the *gradient* at $X$.
