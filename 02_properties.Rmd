...
$$
|\rho(X)-\rho(Y)|=
|\mathop{\sum\sum}_{1\leq i<j\leq n}w_{ij}\delta_{ij}(d_{ij}(X)-d_{ij}(Y))|\leq
...
$$
|d_{ij}(X)-d_{ij}(Y)|=|\|X'(e_i-e_j)\|-\|Y'(e_i-e_j))\||\leq\|(X-Y)'(e_i-e_j)\|\leq\|X-Y\|.
$$
```
...
$$
B(X)X-B(Y)Y=\mathop{\sum\sum}_{1\leq i<j\leq n}w_{ij}\delta_{ij}\left\{\frac{x_i-x_j}{d_{ij}(X)}-\frac{y_i-y_j}{d_{ij}(Y)}\right\`yhhhhhht}(e_i-e_j),
$$
and thus
...
\begin{equation}
\sigma(X+\epsilon
Y)=\sigma(X)-\epsilon\sum_{d_{ij}(X)=0}w_{ij}\delta_{ij}d_{ij}(Y)
...
Theorem \@ref(thm:statpoints) is important, because it means that we can require without loss of generality that $X$ is in the ellipsoidal disk $\eta(X)\leq 1$, which is a compact convex set.
...
```{proof}
By contradiction: suppose that there is a local maximum at $X\not= 0$. Then on the line through zero and $X$ there should be a local maximum at $X$ as well. But we have $$
\sigma(\alpha X)=1-2\alpha\rho(X)+\alpha^2\eta^2(X),
$$ a convex quadratic, which consequently cannot have a local maximum at $X$.

At zero we have for the one-sided directional derivative 
...
which implies that the function is not Gateaux differentiable at zero and has a local maximum there.
```
...
\begin{equation}
d_{ij}(X)=\max_Y \frac{\text{tr}\ X'A_{ij}Y}{d_{ij}(Y)},
(\#eq:dasmax)
\end{equation}
...
\begin{equation}
\sigma(X)=\min_Y\left(1-\text{tr}\ X'B(Y)Y+\frac12\text{tr}\ X'VX\right),
(\#eq:sigasmin)
\end{equation}
